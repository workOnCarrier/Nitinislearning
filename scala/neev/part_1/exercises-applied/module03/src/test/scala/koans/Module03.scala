...
class Module03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  class ComplexNum (val real: Double, val imaginary: Double = 0){
    ...
    override def toString(): String= {
      "" + this.real + (if (imaginary<0)" - " else " + ") + math.abs(this.imaginary) + "i"
    }
  }
  object ComplexNum{
    def apply(real: Double, imaginary: Double): ComplexNum =
      new ComplexNum(real, imaginary)

    implicit def apply(real: Double) : ComplexNum = new ComplexNum(real, 0)
  }
