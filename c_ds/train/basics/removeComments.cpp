...
public:
    vector<string> removeComments(vector<string>& source) {
        ...
        auto HandleLineC = [&result](string& line, auto offset){
            auto remaining = line.substr(0, offset);
            return remaining;
        };
        std::function<string(string&, size_t, int&)> HandleBlockC ;
        HandleBlockC = [&source, endBlockComment, startBlockComment, &HandleBlockC, HandleLineC, lineComment]
                        ( string& line, size_t offset, int& index) {
            auto remaining = line.substr(0, offset);
            offset += 2;
            while (true){
                auto endloc = line.find(endBlockComment, offset+2);
                if ( endloc != string::npos ){
                    auto ending = line.substr(endloc+2, line.length() - (endloc+2) );
                    auto blkCOffset = ending.find(startBlockComment);
                    auto lineCOffset = ending.find(lineComment);
                    if ( lineCOffset < blkCOffset) {
                        ending = HandleLineC(ending, lineCOffset);
                    }else{
                        ending = HandleBlockC ( ending, blkCOffset, index);
                    }
                    remaining += ending;
                    break;
                }
        ...
        for ( int index = 0; index < source.size(); ++index ){
            auto line = source[index];
            auto lineCloc = line.find(lineComment);
            auto blkStart = line.find(startBlockComment);
            if ( lineCloc == blkStart){ // this can happen only when both are npos
                result.push_back(line);
            } else {
                if ( lineCloc != string::npos && blkStart != string::npos){ // both are found in the same line
                    if ( lineCloc < blkStart) {
                        auto remaining = HandleLineC(line, lineCloc);
                        if (remaining.length() > 0) result.push_back(remaining);
                    }else {
                        string blkLessLine = HandleBlockC (line, blkStart, index);
                        auto lineCloc = blkLessLine.find(lineComment);
                        if ( lineCloc != string::npos){
                            auto remaining = HandleLineC (blkLessLine , lineCloc);
                            if (remaining.length() > 0) result.push_back(remaining);
                        } else {
                            if ( blkLessLine.length() > 0) result.push_back(blkLessLine);
                        }
                    }
                } else if ( lineCloc != string::npos) {
                    auto remaining = HandleLineC (line, lineCloc);
                    if (remaining.length() > 0) result.push_back(remaining);
                } else { // handle block start
                    string blkLessLine = HandleBlockC (line, blkStart, index);
                    if ( blkLessLine.length() > 0) result.push_back(blkLessLine);
                }
            }
...
void run( vector<string> input){
    ...
    Solution s;
    auto result = s.removeComments(input);
    cout << "\t result:" << result << endl;
}

void test(){
    // run({"/*Test program */", "int main()", "{ ", "  // variable declaration ", "int a, b, c;", "/* This is a test", "   multiline  ", "   comment for ", "   testing */", "a = b + c;", "}"});
    // run({"struct Node{", "    /*/ declare members;/**/", "    int size;", "    /**/int val;", "};"});
    // run( {"a//*b//*c","blank","d/*/e*//f"});
    // run( {"a/*/b//*c","blank","d//*e/*/f"}); 
    // run({"a/*/b//*c","blank","d/*/e*//f"});
    run({   "/*/dadb/*/aec*////*//*ee*//*//b*////*badbda//*bbacdbbd*//ceb//*cdd//**//de*////*",
